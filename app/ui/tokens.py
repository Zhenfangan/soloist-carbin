...
# 设计画布逻辑尺寸 —— 全项目所有裸像素值(GRID_UNIT/字号/卡片高度等)均按此画布设计。
# 桌面端通过 Config.set("graphics", "width/height", ...) 把窗口锁定为该尺寸，
# 因此裸像素天然对齐；真机不支持锁定窗口尺寸(系统给原生分辨率)，故由
# main.py 用 Scatter 把内容区按 Window 实际尺寸整体等比缩放，缩放基准即此常量。
LOGICAL_WIDTH: int = 420
LOGICAL_HEIGHT: int = 750
