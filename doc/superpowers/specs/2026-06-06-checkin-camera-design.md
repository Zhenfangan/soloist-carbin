...
## 10. Open Questions — 留给 andy 审查时回答

1. **photos 目录是否纳入备份**? 现有"备份数据"功能导出 DB 快照 + 关键配置。photo 目录可能上 GB 级, 全部入备份会让备份文件很大。建议: 备份时只导出"最近 30 天"的 photos, 旧的不导。如果你想全备份, 我改方案。

2. **照片质量与尺寸**? Android 原生 Intent 拍的图可能是 4MB+ 高清原图。要不要在 `take_photo` 内部统一压缩到 720p / 600KB? 不压缩则磁盘占用大。建议: 自动压缩到 720p。

3. **打卡时段的"上班/下班"动作 UI**? 现有 PeriodCard 是怎样的 UI? 是单个按钮 (打卡) 还是双按钮 (上班/下班)? 我没仔细查; 如果只有一个按钮, 那 "in/out" 该怎么区分? 这是 wave-camera 实现阶段必须弄清楚, 但 spec 这里不深入。
